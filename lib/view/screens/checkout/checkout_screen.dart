...
import 'package:flutter_restaurant/data/model/response/table_model.dart';
...
import 'package:flutter_restaurant/provider/table_provider.dart';
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  List<DropdownMenuItem<String>> generateTableList(List<TableModel> tableList,int branchid){
    List<DropdownMenuItem<String>> tbllist = [];
    if(tableList.isEmpty){
      return null;
    }
    tableList.asMap().forEach((ind,tbll) {
      if(tbll.branchId == branchid){
        tbllist.add(
          DropdownMenuItem(
            value: ind.toString(),
            child: Text("${tbll.number} (Max Capacity: ${tbll.capacity})"),
          )
        ); 
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    final _tableList= Provider.of<TableProvider>(context, listen: false).tableList;
    ...
    return Scaffold(
      ...
      body: _isLoggedIn ? Consumer<OrderProvider>(
        builder: (context, order, child) {
          ...
          if((!_takeAway && order.orderType != 'table_order') && _kmWiseCharge) {
          ...
          }else if((!_takeAway  && order.orderType != 'table_order')&& !_kmWiseCharge) {
            ...
            builder: (context, address, child) {
              return Column(
                children: [
                                    ...
                                    children: [
                                      Expanded(
                                        ...
                                        child: Container(
                                          ...
                                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                            ...
                                            // Address
                                            !_takeAway && order.orderType != 'table_order' ? Column(children: [
                                              Padding(
                                            ...
                                            //Added by Me
                                            order.orderType == 'table_order' ?
                                            Column(
                                              children: [
                                                Text("Table Information", style: rubikMedium.copyWith(fontSize: Dimensions.FONT_SIZE_LARGE)),
                                                Container(
                                                  ...
                                                  child: DecoratedBox(
                                                    ...
                                                    child:Padding(
                                                      ...
                                                      child: DropdownButton(        
                                                            // Initial Value
                                                            value: tblind == null ? null:tblind.toString() ,
                                                            hint: tblind == null ? Text("Choose a Table"):SizedBox(),
                                                            // Down Arrow Icon
                                                            // icon: const Icon(Icons.keyboard_arrow_down),    
                                                            disabledHint: Text("No table available"),
                                                            // Array list of items
                                                            // items: _tableList.asMap().map((ind,tbll) {
                                                            //   return new MapEntry(ind, DropdownMenuItem(
                                                            //     value: ind.toString(),
                                                            //     child: Text("${tbll.number} (Max Capacity: ${tbll.capacity})"),
                                                            //   ));
                                                            // }).values.toList(),
                                                            items: generateTableList(_tableList, _branches[order.branchIndex].id),
                                                            // After selecting the desired option,it will
                                                            // change button value to selected value
                                                            onChanged: (String newValue) { 
                                                              setState(() {
                                                                tblind = int.parse(newValue);
                                                              });
                                                            },
                                            ...
                                            :SizedBox(),
                                            // Time Slot
                                            order.orderType != 'table_order'? Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                            ...
                                            if (!ResponsiveHelper.isDesktop(context))  detailsWidget(context, _kmWiseCharge, _takeAway, order, _deliveryCharge, address,_tableList),
                                      ...
                                      if(ResponsiveHelper.isDesktop(context)) Expanded(
                                        ...
                                        child: Container(
                                          ...
                                          child: detailsWidget(
                                              context, _kmWiseCharge, _takeAway, order, _deliveryCharge, address,_tableList),
                                        ),
                                      ),
                                    ],
                                  ),
                  ...
                  if(!ResponsiveHelper.isDesktop(context))  confirmButtonWidget(order, _takeAway, address, _kmWiseCharge, _deliveryCharge, context,_tableList),
  ...
  Container confirmButtonWidget(OrderProvider order, bool _takeAway, LocationProvider address, bool _kmWiseCharge, double _deliveryCharge, BuildContext context,List<TableModel> _tableList) {
    return Container(
      width: 1170,
      alignment: Alignment.center,
      padding: EdgeInsets.all(Dimensions.PADDING_SIZE_SMALL),
      child: !order.isLoading ? Builder(
        builder: (context) => CustomButton(btnTxt: getTranslated('confirm_order', context),
            onTap: () {
              ...
              if(widget.amount < Provider.of<SplashProvider>(context, listen: false).configModel.minimumOrderValue) {
              ...
              }else if((!_takeAway && order.orderType != 'table_order') && (address.addressList == null || address.addressList.length == 0 || order.addressIndex < 0)) {
              ...
              }else if ((!_takeAway && order.orderType != 'table_order')&& _kmWiseCharge && order.distance == -1) {
                showCustomSnackBar(getTranslated('delivery_fee_not_set_yet', context), context);
              }else if(order.orderType == 'table_order' && tblind == null){
                showCustomSnackBar('Please choose a table', context);
              }else if(order.orderType == 'table_order' && _capacityController.text.isEmpty){
                showCustomSnackBar('Please enter the number of people', context);
              }else if(order.orderType == 'table_order' && _capacityController.text.isNotEmpty && _tableList[tblind].capacity < int.parse(_capacityController.text)){
                showCustomSnackBar('Number of people exceeds max capacity', context);
              }
              else {
                List<Cart> carts = [];
                for (int index = 0; index < _cartList.length; index++) {
                  ...
                  List<ProItem> items = [];
                  cart.itemlist.forEach((itm) {
                    ProItem proditem = new ProItem(name: itm.item.name,quantity: itm.quantity);
                    items.add(proditem);
                  });
                  carts.add(Cart(
                    ...
                    items,cart.cartSide,cart.cartDrink,cart.cartDip,cart.isMeal,cart.itemsCost.toString()
                  ));
                }
                PlaceOrderBody _placeOrderBody = PlaceOrderBody(
                  ...
                  deliveryAddressId: (!_takeAway && order.orderType != 'table_order') ? Provider.of<LocationProvider>(context, listen: false)
                  ...
                  tableId: tblind!=null?_tableList[tblind].id:null,numberOfPeople:_capacityController.text.isNotEmpty?int.parse(_capacityController.text):null
                );
                ...
                if(_isCashOnDeliveryActive && Provider.of<OrderProvider>(context, listen: false).paymentMethodIndex == 0) {
                  if(order.orderType == 'table_order'){
                    order.placeTableOrder(_placeOrderBody, _callback);
                  }else{
  ...
  Column detailsWidget(BuildContext context, bool _kmWiseCharge, bool _takeAway, OrderProvider order, double _deliveryCharge, LocationProvider address,List<TableModel> tableList) {
    return Column(
      children: [
        Padding(
          padding: EdgeInsets.symmetric(horizontal: Dimensions.PADDING_SIZE_SMALL),
        ...
        if(ResponsiveHelper.isDesktop(context))  confirmButtonWidget(order, _takeAway, address, _kmWiseCharge, _deliveryCharge, context,tableList),
      ],
    );
