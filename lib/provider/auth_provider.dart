...
class AuthProvider with ChangeNotifier {
  ...
  Future appleLogin(AppleLoginModel applelLogin, Function callback) async {
    print('============== token ==========');
    print(applelLogin.token);
    print(applelLogin.email);
    print(applelLogin.uniqueId);
    print(applelLogin.medium);
    print(applelLogin.firstName);
    print(applelLogin.lastName);
    print('============== token ==========');

    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await authRepo.appleLogin(applelLogin);
    _isLoading = false;
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      Map map = apiResponse.response.data;
      String message = '';
