...
class OrderProvider extends ChangeNotifier {
      ...
      apiResponse.response.data.forEach((order) {
        ...
        if(orderModel.orderStatus == 'pending' ||
          ...
          _runningOrderList.add(orderModel);
            ...
            orderModel.orderStatus == 'declined' ||
  ...
  Future<void> placeTableOrder(PlaceOrderBody placeOrderBody, Function callback) async {
    _isLoading = true;
    notifyListeners();
    print('order body : ${placeOrderBody.toJson()}');
    ApiResponse apiResponse = await orderRepo.placeTableOrder(placeOrderBody);
    _isLoading = false;
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
