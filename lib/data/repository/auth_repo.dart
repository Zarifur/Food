...
class AuthRepo {
  ...
  Future<ApiResponse> appleLogin(AppleLoginModel appleLogin) async {

    print('------apple login -- > ${appleLogin.toJson()}');
    print('------apple gmail -- > ${appleLogin.email}');

    try {
      Response response = await dioClient.post(
        AppConstants.SOCIAL_LOGIN,
        data: appleLogin.toJson(),
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
